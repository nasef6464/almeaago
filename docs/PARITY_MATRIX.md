...
| Identity/Auth | identity | core + recovery + auth UI + phone/password + Google/WhatsApp implemented | session/CSRF/lockout/OTP guards implemented; live provider config pending | normalized identity/session/recovery/provider/OTP schema | legacy auth/OTP flow implemented; screenshot gate pending | provider branch CI pending; live provider smoke pending | IN_PROGRESS |
