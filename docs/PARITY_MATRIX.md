...
Status values: NOT_STARTED · DISCOVERED · SPECIFIED · IMPLEMENTED · TESTED · PARITY_PROVEN · BLOCKED · INTENTIONALLY_CHANGED.
...
| Identity/Auth | identity | core + recovery + phone/password + Google/WhatsApp tested; admin account lifecycle implemented | session/CSRF/lockout/OTP tested; admin-only account mutations implemented; organization scopes pending | normalized identity/session/recovery/provider/OTP/admin-audit schema | legacy auth/OTP UI build-tested; screenshot gate pending | provider gates green; admin backend/database gates pending; live provider smoke pending | IN_PROGRESS |
...
| Question Bank | questionbank/media | pending | pending | identity/skills seed only | pending | pending | NOT_STARTED |
| Assessment | assessment | pending | pending | pending | pending | pending | NOT_STARTED |
