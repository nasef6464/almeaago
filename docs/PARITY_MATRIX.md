...
| Assessment | assessment | definition/version API + staff builder merged; learner start/autosave/resume/submit/scoring implemented in PR #44 | staff scope plus student-only attempt ownership, CSRF writes, idempotency, expiry and server scoring tested | exact published/question versions; transactional attempts/answers/results; no copied question payload state | staff responsive parity plus learner mobile attempt/result checkpoint green | PR #44 implementation checkpoint Backend + Frontend + E2E green; final exact-head closure pending | IN_PROGRESS |
