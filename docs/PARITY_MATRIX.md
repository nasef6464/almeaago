...
| Assessment | assessment | definition/version API implemented on PR #42; execution/distribution remains later | admin/teacher scope, CSRF, workflow, publication and optimistic revision implemented | normalized definition/version/distribution/attempt/answer/result schema tested; API references exact question versions | pending | definition API Backend CI green on implementation checkpoint; final exact-head closure gate pending | IN_PROGRESS |
