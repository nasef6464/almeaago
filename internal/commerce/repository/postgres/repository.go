...
func (r *Repository) auditTx(ctx context.Context, tx pgx.Tx, e operations.AuditEvent) error {
	if r.audit == nil {
		return errors.New("commerce audit writer is not configured")
	}
...
func scanProduct(row scanner) (commerce.Product, error) {
	...
	if packageID != "" && kind != nil {
		p.Package = &commerce.Package{ID: packageID, ProductID: p.ID, PackageKind: *kind, SeatCapacity: seat, ValidityDays: days}
	}
	return p, nil
}
...
`, p.Package.ID)
		if e != nil {
			return commerce.Product{}, e
		}
		defer rows.Close()
		for rows.Next() {
			var item commerce.PackageItem
			if e = rows.Scan(&item.ScopeType, &item.CourseID, &item.PathID, &item.SubjectID, &item.ContentType); e != nil {
				return commerce.Product{}, e
			}
			p.Package.Items = append(p.Package.Items, item)
		}
		if e = rows.Err(); e != nil {
			return commerce.Product{}, e
		}
...
`, w.Code, string(w.ProductType), w.Name, w.Description, string(w.Status), string(w.AccessMode), w.PriceMinor, w.Currency, w.CourseID, w.IsVisible, actor).Scan(&id)
	if err != nil {
		return commerce.Product{}, mapError(err)
	}
	if err = insertPackage(ctx, tx, id, w); err != nil {
		return commerce.Product{}, err
	}
	if err = r.auditTx(ctx, tx, operations.AuditEvent{ActorUserID: actor, Action: "commerce.product.create", ResourceType: "commerce_product", ResourceID: id, Metadata: map[string]any{"type": w.ProductType, "courseId": w.CourseID}}); err != nil {
		return commerce.Product{}, err
	}
...
`, id, expected, w.Code, w.Name, w.Description, string(w.Status), string(w.AccessMode), w.PriceMinor, w.Currency, w.IsVisible, string(w.ProductType), w.CourseID)
	...
	if tag.RowsAffected() == 0 {
		return commerce.Product{}, commerce.ErrVersionConflict
	}
	if w.Package != nil {
		var packageID string
		err = tx.QueryRow(ctx, `
...
`, id, string(w.Package.PackageKind), w.Package.SeatCapacity, w.Package.ValidityDays).Scan(&packageID)
		...
		if _, err = tx.Exec(ctx, `DELETE FROM commerce_package_items WHERE package_id=$1::uuid`, packageID); err != nil {
			return commerce.Product{}, err
		}
		for _, item := range w.Package.Items {
			_, err = tx.Exec(ctx, `
...
`, packageID, string(item.ScopeType), item.CourseID, item.PathID, item.SubjectID, string(item.ContentType))
	...
	if err = r.auditTx(ctx, tx, operations.AuditEvent{ActorUserID: actor, Action: "commerce.product.update", ResourceType: "commerce_product", ResourceID: id, Metadata: map[string]any{"revision": expected + 1, "status": w.Status, "accessMode": w.AccessMode}}); err != nil {
		return commerce.Product{}, err
	}
	if err = tx.Commit(ctx); err != nil {
		return commerce.Product{}, err
...
`, string(subjectType), subjectID, limit+1, (page-1)*limit)
	...
	defer rows.Close()
	out := commerce.EntitlementPage{Page: page, Limit: limit}
	for rows.Next() {
		e, x := scanEntitlement(rows)
		if x != nil {
			return out, x
		}
...
`, string(in.SubjectType), in.UserID, in.SchoolID, in.ProductID, in.IdempotencyKey, actor, *in.StartsAt, in.ExpiresAt).Scan(&id)
	if err != nil {
		return commerce.Entitlement{}, mapError(err)
	}
	if err = r.auditTx(ctx, tx, operations.AuditEvent{ActorUserID: actor, Action: "commerce.entitlement.grant", ResourceType: "commerce_entitlement", ResourceID: id, Metadata: map[string]any{"subjectType": in.SubjectType, "userId": in.UserID, "schoolId": in.SchoolID, "productId": in.ProductID}}); err != nil {
		return commerce.Entitlement{}, err
	}
	if err = tx.Commit(ctx); err != nil {
		return commerce.Entitlement{}, err
...
`, id, expected, actor, reason)
	...
	if tag.RowsAffected() == 0 {
		return commerce.Entitlement{}, commerce.ErrVersionConflict
	}
	if err = r.auditTx(ctx, tx, operations.AuditEvent{ActorUserID: actor, Action: "commerce.entitlement.revoke", ResourceType: "commerce_entitlement", ResourceID: id, Metadata: map[string]any{"reason": reason}}); err != nil {
		return commerce.Entitlement{}, err
	}
	if err = tx.Commit(ctx); err != nil {
		return commerce.Entitlement{}, err
...
`, courseID).Scan(&decision.ProductID, &mode)
	if errors.Is(err, pgx.ErrNoRows) {
		return commerce.AccessDecision{Configured: false, Allowed: false, Reason: "commerce_unconfigured"}, nil
	}
	if err != nil {
		return decision, err
	}
	decision.Configured = true
	if mode == commerce.AccessFree {
		decision.Allowed = true
		decision.Reason = "free_product"
		return decision, nil
	}
