...
func (r *repoStub) ListProducts(context.Context, int, int, commerce.ProductType, commerce.ProductStatus, string) (commerce.ProductPage, error) {
	return r.page, nil
}
func (r *repoStub) GetProduct(context.Context, string) (commerce.Product, error) {
	return r.product, nil
}
func (r *repoStub) CreateProduct(context.Context, string, commerce.ProductWrite) (commerce.Product, error) {
	return r.product, nil
}
func (r *repoStub) UpdateProduct(context.Context, string, string, int, commerce.ProductWrite) (commerce.Product, error) {
	return r.product, nil
}
func (r *repoStub) ListEntitlements(context.Context, int, int, commerce.SubjectType, string) (commerce.EntitlementPage, error) {
	return r.entitlements, nil
}
func (r *repoStub) GrantEntitlement(_ context.Context, _ string, in commerce.EntitlementGrant) (commerce.Entitlement, error) {
	r.lastGrant = in
	return r.entitlement, nil
}
func (r *repoStub) RevokeEntitlement(context.Context, string, string, int, string) (commerce.Entitlement, error) {
	return r.entitlement, nil
}
func (r *repoStub) ResolveCourseAccess(_ context.Context, user string, schools []string, course, path, subject string) (commerce.AccessDecision, error) {
	r.lastUser = user
	r.lastSchools = append([]string(nil), schools...)
	r.lastCourse = course
	r.lastPath = path
...
func (c catalogStub) CourseCommerceScope(context.Context, string) (string, string, bool, error) {
	return c.pathID, c.subjectID, c.eligible, nil
}

type taxonomyStub struct{ ok bool }

func (t taxonomyStub) ValidCommerceScope(context.Context, string, string) (bool, error) {
	return t.ok, nil
}

type schoolsStub struct{ ids []string }

func (s schoolsStub) ActiveSchoolIDsForUser(context.Context, string) ([]string, error) {
...
func studentActor() identity.User {
...
func TestCourseProductRequiresCanonicalPublishedCourse(t *testing.T) {
	...
	if !errors.Is(err, commerce.ErrConflict) {
		t.Fatalf("expected canonical course conflict, got %v", err)
	}
}
...
func TestPackageScopeIsValidatedWithoutCopyingContent(t *testing.T) {
	...
	if !errors.Is(err, commerce.ErrConflict) {
		t.Fatalf("expected taxonomy conflict, got %v", err)
	}
}
...
func TestManualGrantIsAdminOnlyAndServerOwned(t *testing.T) {
	...
	if !errors.Is(err, ErrForbidden) {
		t.Fatalf("student grant should be forbidden, got %v", err)
	}
	...
	if err != nil || out.ID != "e-1" {
		t.Fatalf("admin grant failed %#v %v", out, err)
	}
	if r.lastGrant.SubjectType != commerce.SubjectUser || r.lastGrant.UserID != "student-1" || r.lastGrant.ProductID != "course-product" {
		t.Fatalf("unexpected grant %#v", r.lastGrant)
	}
}
...
func TestFreeProductMustHaveZeroPrice(t *testing.T) {
	...
	if !errors.Is(err, ErrInvalidInput) {
		t.Fatalf("expected invalid free price, got %v", err)
	}
}
