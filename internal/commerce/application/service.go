...
func normalizePage(page, limit int, defaultLimit int) (int, int, error) {
	...
	if page < 1 || page > 10000 || limit < 1 || limit > 100 { return 0,0,ErrInvalidInput }
...
func normalizeProductWrite(w *commerce.ProductWrite) error {
	...
	if w.AccessMode==commerce.AccessFree && w.PriceMinor!=0 { return ErrInvalidInput }
	if w.ProductType==commerce.ProductCourse {
		if w.CourseID=="" || w.Package!=nil { return ErrInvalidInput }
		return nil
	}
	...
	if w.Package.SeatCapacity!=nil && *w.Package.SeatCapacity<1 { return ErrInvalidInput }
	if w.Package.ValidityDays!=nil && (*w.Package.ValidityDays<1 || *w.Package.ValidityDays>3650) { return ErrInvalidInput }
	return nil
}
...
func normalizePackageItems(items []commerce.PackageItem) ([]commerce.PackageItem,error) {
	...
	for _,item:=range items{
		...
		if !commerce.ValidScopeType(item.ScopeType){return nil,ErrInvalidInput}
		switch item.ScopeType{
		case commerce.ScopeCourse:
			if item.CourseID==""||item.PathID!=""||item.SubjectID!=""||item.ContentType!=""{return nil,ErrInvalidInput}
		case commerce.ScopePath:
			if item.PathID==""||item.CourseID!=""||item.SubjectID!=""||item.ContentType!=""{return nil,ErrInvalidInput}
		case commerce.ScopeSubject:
			if item.SubjectID==""||item.CourseID!=""||item.PathID!=""||item.ContentType!=""{return nil,ErrInvalidInput}
		case commerce.ScopeContentType:
			if item.CourseID!=""||item.PathID!=""||item.SubjectID!=""||!commerce.ValidContentType(item.ContentType){return nil,ErrInvalidInput}
...
func (s *Service) validateProductTargets(ctx context.Context, w *commerce.ProductWrite) error {
	if w.ProductType==commerce.ProductCourse {
		if s.catalog==nil{return commerce.ErrConflict}
		_,_,eligible,err:=s.catalog.CourseCommerceScope(ctx,w.CourseID)
		if err!=nil{return err}
		if !eligible{return commerce.ErrConflict}
		return nil
	}
	...
	for _,item:=range items{
		...
		case commerce.ScopeCourse:
			...
			if !eligible{return commerce.ErrConflict}
		case commerce.ScopePath:
			if s.taxonomy==nil{return commerce.ErrConflict}
			ok,e:=s.taxonomy.ValidCommerceScope(ctx,item.PathID,"")
			if e!=nil{return e}
			if !ok{return commerce.ErrConflict}
		case commerce.ScopeSubject:
			if s.taxonomy==nil{return commerce.ErrConflict}
...
func (s *Service) ListProducts(ctx context.Context, actor identity.User, page, limit int, kind commerce.ProductType, status commerce.ProductStatus, search string) (commerce.ProductPage,error) {
	if !actor.HasRole(identity.RoleAdmin){return commerce.ProductPage{},ErrForbidden}
	var err error
	page,limit,err=normalizePage(page,limit,50);if err!=nil{return commerce.ProductPage{},err}
	search=strings.TrimSpace(search);if len(search)>160{return commerce.ProductPage{},ErrInvalidInput}
	if kind!=""&&!commerce.ValidProductType(kind){return commerce.ProductPage{},ErrInvalidInput}
	if status!=""&&!commerce.ValidProductStatus(status){return commerce.ProductPage{},ErrInvalidInput}
...
func (s *Service) Product(ctx context.Context, actor identity.User, id string) (commerce.Product,error) {
	if !actor.HasRole(identity.RoleAdmin){return commerce.Product{},ErrForbidden}
	id=strings.TrimSpace(id);if id==""{return commerce.Product{},ErrInvalidInput}
...
func (s *Service) CreateProduct(ctx context.Context, actor identity.User, w commerce.ProductWrite) (commerce.Product,error) {
	if !actor.HasRole(identity.RoleAdmin){return commerce.Product{},ErrForbidden}
	if err:=normalizeProductWrite(&w);err!=nil{return commerce.Product{},err}
	if err:=s.validateProductTargets(ctx,&w);err!=nil{return commerce.Product{},err}
...
func (s *Service) UpdateProduct(ctx context.Context, actor identity.User, id string, expectedRevision int, w commerce.ProductWrite) (commerce.Product,error) {
	if !actor.HasRole(identity.RoleAdmin){return commerce.Product{},ErrForbidden}
	id=strings.TrimSpace(id);if id==""||expectedRevision<1{return commerce.Product{},ErrInvalidInput}
	current,err:=s.repo.GetProduct(ctx,id);if err!=nil{return commerce.Product{},err}
	if err=normalizeProductWrite(&w);err!=nil{return commerce.Product{},err}
	if current.ProductType!=w.ProductType || current.CourseID!=w.CourseID{return commerce.Product{},commerce.ErrConflict}
	if err=s.validateProductTargets(ctx,&w);err!=nil{return commerce.Product{},err}
...
func (s *Service) ListEntitlements(ctx context.Context, actor identity.User, page, limit int, subjectType commerce.SubjectType, subjectID string) (commerce.EntitlementPage,error) {
	if !actor.HasRole(identity.RoleAdmin){return commerce.EntitlementPage{},ErrForbidden}
	var err error
	page,limit,err=normalizePage(page,limit,50);if err!=nil{return commerce.EntitlementPage{},err}
	subjectID=strings.TrimSpace(subjectID)
	if subjectType!=""&&!commerce.ValidSubjectType(subjectType){return commerce.EntitlementPage{},ErrInvalidInput}
	if subjectType!=""&&subjectID==""{return commerce.EntitlementPage{},ErrInvalidInput}
...
func (s *Service) GrantManual(ctx context.Context, actor identity.User, in commerce.EntitlementGrant) (commerce.Entitlement,error) {
	if !actor.HasRole(identity.RoleAdmin){return commerce.Entitlement{},ErrForbidden}
	in.UserID=strings.TrimSpace(in.UserID);in.SchoolID=strings.TrimSpace(in.SchoolID);in.ProductID=strings.TrimSpace(in.ProductID);in.IdempotencyKey=strings.TrimSpace(in.IdempotencyKey)
	if !commerce.ValidSubjectType(in.SubjectType)||in.ProductID==""||in.IdempotencyKey==""||len(in.IdempotencyKey)>160{return commerce.Entitlement{},ErrInvalidInput}
	if (in.SubjectType==commerce.SubjectUser&&(in.UserID==""||in.SchoolID!=""))||(in.SubjectType==commerce.SubjectSchool&&(in.SchoolID==""||in.UserID!="")){return commerce.Entitlement{},ErrInvalidInput}
	if in.StartsAt==nil{now:=time.Now().UTC();in.StartsAt=&now}
	if in.ExpiresAt!=nil&&!in.ExpiresAt.After(*in.StartsAt){return commerce.Entitlement{},ErrInvalidInput}
	product,err:=s.repo.GetProduct(ctx,in.ProductID);if err!=nil{return commerce.Entitlement{},err}
	if product.Status!=commerce.ProductActive{return commerce.Entitlement{},commerce.ErrConflict}
...
func (s *Service) Revoke(ctx context.Context, actor identity.User, id string, expectedRevision int, reason string) (commerce.Entitlement,error) {
	if !actor.HasRole(identity.RoleAdmin){return commerce.Entitlement{},ErrForbidden}
	id=strings.TrimSpace(id);reason=strings.TrimSpace(reason)
	if id==""||expectedRevision<1||reason==""||len(reason)>1000{return commerce.Entitlement{},ErrInvalidInput}
...
func (s *Service) CheckCourseAccess(ctx context.Context, userID, courseID string) (bool,bool,string,error) {
	userID=strings.TrimSpace(userID);courseID=strings.TrimSpace(courseID)
	if userID==""||courseID==""{return false,false,"invalid",ErrInvalidInput}
	if s.catalog==nil{return false,false,"resolver_unavailable",commerce.ErrConflict}
	pathID,subjectID,eligible,err:=s.catalog.CourseCommerceScope(ctx,courseID)
	if err!=nil{return false,false,"",err}
	if !eligible{return false,true,"course_unavailable",nil}
...
func (s *Service) CourseAccess(ctx context.Context, actor identity.User, courseID string) (commerce.AccessDecision,error) {
	if strings.TrimSpace(actor.ID)==""{return commerce.AccessDecision{},ErrForbidden}
	if s.catalog==nil{return commerce.AccessDecision{},commerce.ErrConflict}
	pathID,subjectID,eligible,err:=s.catalog.CourseCommerceScope(ctx,strings.TrimSpace(courseID))
	if err!=nil{return commerce.AccessDecision{},err}
	if !eligible{return commerce.AccessDecision{Configured:true,Allowed:false,Reason:"course_unavailable"},nil}
	var schoolIDs []string
	if s.schools!=nil {
		schoolIDs,err=s.schools.ActiveSchoolIDsForUser(ctx,actor.ID);if err!=nil{return commerce.AccessDecision{},err}
	}
