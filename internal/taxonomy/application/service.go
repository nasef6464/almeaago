...
func (s *Service) CreatePath(ctx context.Context, actor identity.User, write taxonomy.PathWrite) (taxonomy.Path, error) {
	if !actor.HasRole(identity.RoleAdmin) { return taxonomy.Path{}, ErrForbidden }
	...
	if write.Code == "" || write.Name == "" || write.SortOrder < 0 || len(write.Name) > 160 || len(write.Description) > 4000 { return taxonomy.Path{}, ErrInvalidInput }
...
func (s *Service) UpdatePath(ctx context.Context, actor identity.User, pathID string, patch taxonomy.PathPatch) (taxonomy.Path, error) {
	...
	if patch.ParentPathID != nil { v := strings.TrimSpace(*patch.ParentPathID); patch.ParentPathID = &v }
	if pathID == "" || !validCommonPatch(patch.Name, patch.Description, patch.SortOrder, patch.Status, patch.ParentPathID != nil) { return taxonomy.Path{}, ErrInvalidInput }
...
func (s *Service) CreateLevel(ctx context.Context, actor identity.User, write taxonomy.LevelWrite) (taxonomy.Level, error) {
	if !actor.HasRole(identity.RoleAdmin) { return taxonomy.Level{}, ErrForbidden }
	write.PathID, write.Code, write.Name = strings.TrimSpace(write.PathID), normalizeCode(write.Code), strings.TrimSpace(write.Name)
	if write.PathID == "" || write.Code == "" || write.Name == "" || write.SortOrder < 0 || len(write.Name) > 160 { return taxonomy.Level{}, ErrInvalidInput }
...
func (s *Service) UpdateLevel(ctx context.Context, actor identity.User, levelID string, patch taxonomy.LevelPatch) (taxonomy.Level, error) {
	...
	if levelID == "" || !validCommonPatch(patch.Name, nil, patch.SortOrder, patch.Status, false) { return taxonomy.Level{}, ErrInvalidInput }
...
func (s *Service) CreateSubject(ctx context.Context, actor identity.User, write taxonomy.SubjectWrite) (taxonomy.Subject, error) {
	if !actor.HasRole(identity.RoleAdmin) { return taxonomy.Subject{}, ErrForbidden }
	...
	if write.PathID == "" || write.Code == "" || write.Name == "" || write.SortOrder < 0 || len(write.Name) > 160 { return taxonomy.Subject{}, ErrInvalidInput }
...
func (s *Service) UpdateSubject(ctx context.Context, actor identity.User, subjectID string, patch taxonomy.SubjectPatch) (taxonomy.Subject, error) {
	...
	if patch.LevelID != nil { v := strings.TrimSpace(*patch.LevelID); patch.LevelID = &v }
	if subjectID == "" || !validCommonPatch(patch.Name, nil, patch.SortOrder, patch.Status, patch.LevelID != nil) { return taxonomy.Subject{}, ErrInvalidInput }
...
func (s *Service) CreateSkill(ctx context.Context, actor identity.User, write taxonomy.SkillWrite) (taxonomy.Skill, error) {
	if !actor.HasRole(identity.RoleAdmin) { return taxonomy.Skill{}, ErrForbidden }
	...
	if write.SubjectID == "" || write.Code == "" || write.Name == "" || write.SortOrder < 0 || (write.Kind != "main" && write.Kind != "sub") || len(write.Name) > 160 || len(write.Description) > 4000 { return taxonomy.Skill{}, ErrInvalidInput }
	if (write.Kind == "main" && write.ParentSkillID != "") || (write.Kind == "sub" && write.ParentSkillID == "") { return taxonomy.Skill{}, ErrInvalidInput }
...
func (s *Service) UpdateSkill(ctx context.Context, actor identity.User, skillID string, patch taxonomy.SkillPatch) (taxonomy.Skill, error) {
	...
	if patch.ParentSkillID != nil { v := strings.TrimSpace(*patch.ParentSkillID); patch.ParentSkillID = &v }
	if skillID == "" || !validCommonPatch(patch.Name, patch.Description, patch.SortOrder, patch.Status, patch.ParentSkillID != nil) { return taxonomy.Skill{}, ErrInvalidInput }
...
func validCommonPatch(name, description *string, sortOrder *int, status *taxonomy.Status, relationChanged bool) bool {
	if name != nil { v := strings.TrimSpace(*name); *name = v; if v == "" || len(v) > 160 { return false } }
	if description != nil { v := strings.TrimSpace(*description); *description = v; if len(v) > 4000 { return false } }
	if sortOrder != nil && *sortOrder < 0 { return false }
	if status != nil && !taxonomy.ValidStatus(*status) { return false }
	return name != nil || description != nil || sortOrder != nil || status != nil || relationChanged
}

func normalizeCode(value string) string {
	value = strings.ToUpper(strings.TrimSpace(value))
	if len(value) == 0 || len(value) > 64 { return "" }
	return value
}
