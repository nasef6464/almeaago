...
func (h *Handler) createPath(w http.ResponseWriter, r *http.Request) {
	auth, ok := h.authenticateMutation(w, r)
	if !ok {
		return
	}
	var p taxonomy.PathWrite
	if !decodeJSON(w, r, &p) {
		return
	}
	row, err := h.service.CreatePath(r.Context(), auth.User, p)
	writeMutation(w, http.StatusCreated, map[string]any{"path": row}, err)
}
func (h *Handler) updatePath(w http.ResponseWriter, r *http.Request) {
	...
	var p taxonomy.PathPatch
	if !decodeJSON(w, r, &p) {
		return
	}
	row, err := h.service.UpdatePath(r.Context(), auth.User, chi.URLParam(r, "id"), p)
	writeMutation(w, http.StatusOK, map[string]any{"path": row}, err)
}
func (h *Handler) createLevel(w http.ResponseWriter, r *http.Request) {
	...
	var p taxonomy.LevelWrite
	if !decodeJSON(w, r, &p) {
		return
	}
	row, err := h.service.CreateLevel(r.Context(), auth.User, p)
	writeMutation(w, http.StatusCreated, map[string]any{"level": row}, err)
}
func (h *Handler) updateLevel(w http.ResponseWriter, r *http.Request) {
	...
	var p taxonomy.LevelPatch
	if !decodeJSON(w, r, &p) {
		return
	}
	row, err := h.service.UpdateLevel(r.Context(), auth.User, chi.URLParam(r, "id"), p)
	writeMutation(w, http.StatusOK, map[string]any{"level": row}, err)
}
func (h *Handler) createSubject(w http.ResponseWriter, r *http.Request) {
	...
	var p taxonomy.SubjectWrite
	if !decodeJSON(w, r, &p) {
		return
	}
	row, err := h.service.CreateSubject(r.Context(), auth.User, p)
	writeMutation(w, http.StatusCreated, map[string]any{"subject": row}, err)
}
func (h *Handler) updateSubject(w http.ResponseWriter, r *http.Request) {
	...
	var p taxonomy.SubjectPatch
	if !decodeJSON(w, r, &p) {
		return
	}
	row, err := h.service.UpdateSubject(r.Context(), auth.User, chi.URLParam(r, "id"), p)
	writeMutation(w, http.StatusOK, map[string]any{"subject": row}, err)
}
func (h *Handler) createSkill(w http.ResponseWriter, r *http.Request) {
	...
	var p taxonomy.SkillWrite
	if !decodeJSON(w, r, &p) {
		return
	}
	row, err := h.service.CreateSkill(r.Context(), auth.User, p)
	writeMutation(w, http.StatusCreated, map[string]any{"skill": row}, err)
}
func (h *Handler) updateSkill(w http.ResponseWriter, r *http.Request) {
	...
	var p taxonomy.SkillPatch
	if !decodeJSON(w, r, &p) {
		return
	}
	row, err := h.service.UpdateSkill(r.Context(), auth.User, chi.URLParam(r, "id"), p)
	writeMutation(w, http.StatusOK, map[string]any{"skill": row}, err)
}
...
func writeMutation(w http.ResponseWriter, status int, body any, err error) {
	...
	switch {
	case errors.Is(err, taxonomyapp.ErrInvalidInput):
		writeJSON(w, http.StatusBadRequest, map[string]string{"message": "Invalid taxonomy request"})
	case errors.Is(err, taxonomyapp.ErrForbidden):
		writeJSON(w, http.StatusForbidden, map[string]string{"message": "Forbidden"})
	case errors.Is(err, taxonomy.ErrNotFound):
		writeJSON(w, http.StatusNotFound, map[string]string{"message": "Taxonomy record not found"})
	case errors.Is(err, taxonomy.ErrConflict):
		writeJSON(w, http.StatusConflict, map[string]string{"message": "Taxonomy hierarchy conflict"})
	default:
