...
func (r *commerceAccessRepo) GetLearnerCourse(context.Context, string) (content.LearnerCourse, error) {
	return r.course, nil
}
func (r *commerceAccessRepo) GetLearnerCourseLesson(context.Context, string, string) (content.LearnerLessonDetail, error) {
	return r.lesson, nil
}

type courseAccessStub struct {
...
func TestLearnerCourseMarksOnlyNonPreviewLessonsCommerceLocked(t *testing.T) {
	...
	if row.Modules[0].Lessons[0].CommerceLocked {
		t.Fatal("preview must remain accessible")
	}
	if !row.Modules[0].Lessons[1].CommerceLocked {
		t.Fatal("paid lesson must be commerce locked")
	}
	if row.AccessAllowed || !row.AccessConfigured || row.AccessReason != "paid_required" {
		t.Fatalf("unexpected access projection %#v", row)
	}
}
...
func TestLearnerCourseLessonEnforcesCommerceButKeepsPreview(t *testing.T) {
	...
	s := NewServiceWithAuthorScope(repo, repo, courseAccessStub{allowed: false, configured: true, reason: "paid_required"})
	if _, err := s.LearnerCourseLesson(context.Background(), actor, "course-1", "lesson-1"); !errors.Is(err, ErrForbidden) {
		t.Fatalf("expected commerce forbidden, got %v", err)
	}
	repo.lesson.IsPreview = true
	if _, err := s.LearnerCourseLesson(context.Background(), actor, "course-1", "lesson-1"); err != nil {
		t.Fatalf("preview should bypass paid grant: %v", err)
	}
}
