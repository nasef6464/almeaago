...
func (h *Handler) listTopics(w http.ResponseWriter, r *http.Request) {
	...
	query, ok := parseTopicQuery(w, r); if !ok { return }
	page, err := h.service.ListTopics(r.Context(), auth.User, query); if err != nil { writeError(w, err); return }
	items := make([]map[string]any, 0, len(page.Items))
	for _, row := range page.Items { items = append(items, presentTopicSummary(row)) }
...
func (h *Handler) createTopic(w http.ResponseWriter, r *http.Request) {
	...
	var input contentapp.TopicInput; if !decodeJSON(w, r, &input) { return }
	row, err := h.service.CreateTopic(r.Context(), auth.User, input); if err != nil { writeError(w, err); return }
...
func (h *Handler) getTopic(w http.ResponseWriter, r *http.Request) {
	auth, ok := h.authenticate(w, r, false); if !ok { return }
	row, err := h.service.StaffTopic(r.Context(), auth.User, chi.URLParam(r, "id")); if err != nil { writeError(w, err); return }
...
func (h *Handler) updateTopic(w http.ResponseWriter, r *http.Request) {
	auth, ok := h.authenticate(w, r, true); if !ok { return }
	var input contentapp.UpdateTopicInput; if !decodeJSON(w, r, &input) { return }
	row, err := h.service.UpdateTopic(r.Context(), auth.User, chi.URLParam(r, "id"), input); if err != nil { writeError(w, err); return }
	writeJSON(w, http.StatusOK, map[string]any{"topic": presentTopic(row)})
}
