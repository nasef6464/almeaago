...
func (f *fakeService) Register(context.Context, string, string, string) (application.AuthResult, error) {
	return f.authResult, f.authErr
}

func (f *fakeService) Login(context.Context, string, string) (application.AuthResult, error) {
	return f.authResult, f.authErr
}

func (f *fakeService) LoginNationalID(context.Context, string, string) (application.AuthResult, error) {
	return f.authResult, f.authErr
}

func (f *fakeService) LoginPhone(context.Context, string, string) (application.AuthResult, error) {
	return f.authResult, f.authErr
}

func (f *fakeService) Authenticate(context.Context, string) (application.Authenticated, error) {
	return f.auth, f.authErr
}

func (f *fakeService) RotateCSRF(context.Context, application.Authenticated) (string, error) {
...
func (f *fakeService) ResetPassword(context.Context, string, string) error {
	return f.authErr
}

func (f *fakeService) VerifyEmail(context.Context, string) (domain.User, error) {
	return f.auth.User, f.authErr
}

func (f *fakeService) ResendEmailVerification(context.Context, application.Authenticated) error {
	return f.authErr
}

func TestLoginSetsHttpOnlyCookieAndNoStore(t *testing.T) {
	service := &fakeService{
		authResult: application.AuthResult{
			User: domain.User{
...
func TestMeDoesNotExposeNationalIDOrPhone(t *testing.T) {
	service := &fakeService{
		auth: application.Authenticated{
			User: domain.User{
				ID:         "019b0000-0000-7000-8000-000000000001",
	...
	if strings.Contains(body, "nationalId") || strings.Contains(body, "phone") {
		t.Fatalf("generic auth payload leaked identity PII: %s", body)
	}
}

func TestPhonePasswordRoute(t *testing.T) {
	service := &fakeService{
		authResult: application.AuthResult{
			User: domain.User{
				ID:     "user-1",
	...
	req := httptest.NewRequest(
		...
		strings.NewReader("{\"phone\":\"966501234567\",\"password\":\"Password1\"}"),
	)
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
	router.ServeHTTP(rec, req)
