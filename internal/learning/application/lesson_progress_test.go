...
func (r *lessonProgressRepoStub) GetLessonProgress(_ context.Context,student,lessonID string,ct learning.LessonProgressContext,courseID,topicID string)(learning.LessonProgress,error){
	...
	if r.row.LessonID=="" { return learning.LessonProgress{}, learning.ErrNotFound }
	return r.row,nil
}
func (r *lessonProgressRepoStub) SaveVideoProgress(_ context.Context,student,lessonID string,ct learning.LessonProgressContext,courseID,topicID string,pos int)(learning.LessonProgress,error){
	r.lastStudent=student;r.lastPosition=pos
...
func TestLessonProgressRequiresStudentAndExactContext(t *testing.T){
	...
	if _,err:=s.Get(context.Background(),teacher,"lesson-1",learning.LessonProgressContextInput{ContextType:learning.LessonProgressCourse,CourseID:"course-1"});!errors.Is(err,ErrForbidden){t.Fatalf("want forbidden got %v",err)}
	if _,err:=s.Get(context.Background(),progressStudent(),"lesson-1",learning.LessonProgressContextInput{ContextType:learning.LessonProgressCourse});!errors.Is(err,ErrInvalidInput){t.Fatalf("want exact course context got %v",err)}
}
...
func TestLessonProgressReadReturnsNotStartedWithoutCreatingRow(t *testing.T){
	...
	if out.Status!=learning.LessonNotStarted||out.PositionSeconds!=0{t.Fatalf("unexpected default: %#v",out)}
}
...
func TestVideoProgressIsBoundedAndDoesNotAutoComplete(t *testing.T){
	...
	if err!=nil{t.Fatal(err)}
	if repo.lastPosition!=120||out.Status!=learning.LessonInProgress{t.Fatalf("expected capped non-completing progress: repo=%d out=%#v",repo.lastPosition,out)}
}
...
func TestVideoProgressRejectsNonVideoAndHiddenTarget(t *testing.T){
	...
	if _,err:=s.SaveVideo(context.Background(),progressStudent(),"lesson-1",learning.VideoProgressWrite{LessonProgressContextInput:ctx,PositionSeconds:10});!errors.Is(err,ErrInvalidInput){t.Fatalf("non-video should fail: %v",err)}
	s=NewLessonProgressService(&lessonProgressRepoStub{},lessonTargetStub{ok:false,lessonType:"video"})
	if _,err:=s.Complete(context.Background(),progressStudent(),"lesson-1",ctx);!errors.Is(err,ErrNotFound){t.Fatalf("hidden target should fail closed: %v",err)}
}
