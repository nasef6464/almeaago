...
export const commerceClient={
 ...
 recordPaymentReversal:(row:CommercePaymentRequest,reversalType:'refund'|'chargeback',providerReference:string,evidence:string,csrf:string)=>req<{result:{paymentRequest:CommercePaymentRequest;reversal:CommercePaymentReversal;duplicate:boolean}}>(`/api/v1/commerce/admin/payment-requests/${encodeURIComponent(row.id)}/reversal`,{method:'PATCH',headers:{'Content-Type':'application/json','X-CSRF-Token':csrf},body:JSON.stringify({expectedRevision:row.revision,reversalType,providerReference,evidence})}),
