  ...
  return <main dir="rtl" className="min-h-[calc(100vh-5rem)] bg-gray-50 px-3 py-5 sm:px-6">
    <div className="mx-auto grid max-w-6xl gap-4 lg:grid-cols-[300px_1fr]">
      ...
      <section className="space-y-4">
        ...
        {!course.access.allowed?<div data-testid="course-commerce-lock" className="rounded-xl border border-amber-100 bg-amber-50 p-4 text-sm font-bold leading-7 text-amber-900"><div>{course.access.configured?'هذه الدورة مدفوعة وتحتاج صلاحية وصول فعالة. يمكنك مشاهدة المعاينات المجانية فقط.':'سياسة الوصول التجارية لهذه الدورة غير مكتملة؛ تم قفل المحتوى غير المجاني احترازيًا.'}</div>{course.access.configured&&course.access.productId?<Link to={`/checkout/${course.access.productId}`} className="mt-3 inline-flex rounded-xl bg-slate-950 px-4 py-2.5 text-sm font-black text-white">طلب شراء الدورة</Link>:null}</div>:null}
