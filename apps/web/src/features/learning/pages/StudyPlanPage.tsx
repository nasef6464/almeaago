  ...
  return <main dir="rtl" className="min-h-[calc(100vh-5rem)] bg-gray-50 px-3 py-6 sm:px-6">
    <div className="mx-auto max-w-6xl space-y-5">
      ...
      {statusView==='active'?<section className="space-y-4 rounded-3xl border bg-white p-4 shadow-sm sm:p-6">
        ...
        <div><div className="mb-2 text-sm font-black">المواد <span className="text-xs font-bold text-gray-400">(فارغ = كل مواد المسار)</span></div><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{subjects.map(s=><label key={s.id} className={`flex items-center gap-2 rounded-xl border p-3 text-sm font-bold ${draft.subjectIds.includes(s.id)?'border-indigo-300 bg-indigo-50':''}`}><input aria-label={`اختر مادة ${s.name}`} type="checkbox" checked={draft.subjectIds.includes(s.id)} onChange={()=>toggleSubject(s.id)}/>{s.name}</label>)}</div></div>

        <div className="space-y-2"><div className="text-sm font-black">دورات محددة <span className="text-xs font-bold text-gray-400">(اختياري، التحميل حسب المادة فقط)</span></div><select aria-label="مادة اختيار الدورات" value={courseSubjectId} onChange={e=>setCourseSubjectId(e.target.value)} className="w-full rounded-xl border p-2.5"><option value="">اختر مادة لعرض دوراتها</option>{subjects.filter(s=>draft.subjectIds.length===0||draft.subjectIds.includes(s.id)).map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select>{courseOptions.length?<div className="grid gap-2 sm:grid-cols-2">{courseOptions.map(c=><label key={c.id} className={`flex items-center gap-2 rounded-xl border p-3 text-sm font-bold ${draft.courseIds.includes(c.id)?'border-emerald-300 bg-emerald-50':''}`}><input aria-label={`اختر دورة ${c.title}`} type="checkbox" checked={draft.courseIds.includes(c.id)} onChange={()=>toggleCourse(c.id)}/>{c.title}</label>)}</div>:null}</div>
